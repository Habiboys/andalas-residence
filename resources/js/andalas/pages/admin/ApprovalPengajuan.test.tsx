...
it('shows evidence links and review decisions for a paid legacy application', () => {
...
it('keeps issued letters downloadable without an approval form', () => {
    const html = renderToStaticMarkup(
        <ApprovalPengajuan
            bebas_asrama={[
                {
    ...
    expect(html).toContain('Unduh surat');
    expect(html).not.toContain('<form');
});
it('shows an empty state', () => {
    expect(renderToStaticMarkup(<ApprovalPengajuan />)).toContain(
        'Belum ada pengajuan.',
    );
});
