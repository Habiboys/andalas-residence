...
const columns: TableColumn<(typeof rows)[number]>[] = [
    {
        key: 'name',
        label: 'Nama',
        render: (row) => <strong>{row.profile.name}</strong>,
    },
    ...
    { key: 'status', label: 'Status', filter: { type: 'select' } },
    {
        key: 'date',
        label: 'Tanggal',
        render: (row) => row.date.split('-').reverse().join('/'),
    },
...
describe('table search, filtering and ordering', () => {
    ...
    it('combines text and exact select filters before ordering', () => {
        expect(
            processTableRows(rows, columns, {
                ...options,
                columnFilters: { status: 'pending', name: 'budi' },
            }).map((row) => row.id),
        ).toEqual(['c']);
    });
    ...
    it('does not match the action labels', () => {
        expect(
            processTableRows(rows, columns, { ...options, search: 'hapus' }),
        ).toEqual([]);
    });
        ...
        const computed = [
            {
                key: 'double',
                label: 'Double',
                value: (row: (typeof rows)[number]) => row.nominal * 2,
            },
        ];
        expect(
            processTableRows(rows, computed, {
                ...options,
                columnFilters: { double: '1800' },
            }).map((row) => row.id),
        ).toEqual(['c']);
