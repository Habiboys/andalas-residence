...
export function PerformanceSummary({ stats }: Props) {
    if (stats.length === 0) {
        return (
            <p className="p-4 text-sm text-muted">
                Belum ada penilaian teknisi. Skor muncul setelah kuesioner pertama dikirim.
            </p>
        );
    }
    ...
    return (
        <div className="grid gap-4 p-4 sm:grid-cols-2">
            {stats.map((tek) => (
                <div key={tek.teknisi_id} className="rounded-box border border-base-300 p-4">
                    <h3 className="text-sm font-medium">{tek.nama}</h3>
                    <p className="text-identifier mt-0.5 text-xs text-muted">{tek.nim_nip}</p>
