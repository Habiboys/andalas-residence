...
export function AndalasAdminNav() {
    const page = usePage();
    const authUser = (page.props as { auth?: { user?: AuthUser | null } }).auth
        ?.user;
    const isSuperadmin =
        authUser?.roles?.some((r) => r.name === 'superadmin') ?? false;
            ...
            {Object.entries(groups).map(([group, items]) => (
                <SidebarGroup key={group || 'main'} className="px-2 py-0">
                    {group ? (
                        ...
                        <SidebarGroupLabel>Andalas Residen</SidebarGroupLabel>
                    )}
                    <SidebarMenu>
                        {items.map((item) => (
